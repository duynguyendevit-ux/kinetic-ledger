...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 h-full w-64 flex flex-col z-40 bg-[#0b1326] border-r border-[#222a3d]/50 shadow-[4px_0_24px_rgba(0,0,0,0.3)]">
        <div className="p-6">
        ...
        <nav className="flex-1 px-4 space-y-1">
          <a className="flex items-center space-x-3 py-3 px-4 rounded-lg bg-[#222a3d] text-[#00e5ff] font-bold border-l-4 border-[#00e5ff]" href="#">
            <span className="material-symbols-outlined">dashboard</span>
            <span>Dashboard</span>
          </a>
          <a className="flex items-center space-x-3 py-3 px-4 text-[#bac9cc] hover:text-[#c3f5ff] hover:bg-[#131b2e] transition-colors duration-200 rounded-lg" href="#">
      ...
      <main className="ml-64 min-h-screen">
        {/* Header */}
        <header className="sticky top-0 z-30 flex items-center justify-between px-8 h-20 bg-[#0b1326]/80 backdrop-blur-xl border-b border-[#222a3d]/30">
          <div className="flex items-center space-x-8">
            <div className="relative">
              ...
              <input 
                className="bg-surface-container border-none rounded-full pl-10 pr-4 py-2 text-sm focus:ring-1 focus:ring-primary w-64" 
          ...
          <div className="flex items-center space-x-4">
            <button className="bg-primary-container text-on-primary-container px-4 py-2 font-bold rounded transition-all">
              Add API Key
            </button>
          </div>
        </header>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-surface-container-high p-6 rounded-xl border-l-2 border-primary relative overflow-hidden group">
                  ...
                  <p className="font-headline text-4xl font-bold text-on-surface font-mono">
                    {(totalTokens / 1000000).toFixed(1)}M
                  </p>
                </div>

                <div className="bg-surface-container-high p-6 rounded-xl border-l-2 border-tertiary relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                ...
                <div className="bg-surface-container-high p-6 rounded-xl border-l-2 border-primary-container relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                      ...
                      {Object.entries(usage?.byModel || {}).map(([model, stats]) => (
                        <tr key={model} className="hover:bg-surface-container-high/50 transition-colors">
                          <td className="px-6 py-5">
                            <div className="text-sm font-bold text-on-surface">{model}</div>
                          </td>
                          <td className="px-6 py-5 font-mono text-sm text-on-surface">
                            {stats.requests.toLocaleString()}
                          </td>
                          <td className="px-6 py-5 font-mono text-sm text-primary">
              ...
              <div className="bg-surface-container-high p-8 rounded-2xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {combos.map((combo) => (
                    <div key={combo.id} className="p-6 bg-surface-container-highest rounded-xl">
                      <p className="text-sm font-bold text-on-surface mb-4">{combo.name}</p>
